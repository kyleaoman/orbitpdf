...
int sort_by_id(const void *a, const void *b) {
  const halo_index_key *c = a;
  const halo_index_key *d = b;
...
halo_list *_lookup_or_create_scale(float scale) {
  int64_t index = scale*SCALE_FACTOR_MUL;
  int64_t index2;
  assert(index >= 0);
  ...
  if (index2 < 0) {
    halo_tree.halo_lists = check_realloc(halo_tree.halo_lists,
					 sizeof(halo_list)*(halo_tree.num_lists+1),
					 "Halo Lists");
    index2 = halo_tree.num_lists;
    memset(&(halo_tree.halo_lists[index2]), 0, sizeof(halo_list));
...
void build_halo_index(halo_list *hl) {
  int64_t i;
  hl->halo_lookup = check_realloc(hl->halo_lookup, 
				  sizeof(halo_index_key)*hl->num_halos, "Halo lookup index.");
  for (i=0; i<hl->num_halos; i++) {
    hl->halo_lookup[i].id = hl->halos[i].id;
    hl->halo_lookup[i].index = i;
  }
  qsort(hl->halo_lookup, hl->num_halos, sizeof(halo_index_key), sort_by_id);
}
...
void build_tree(void) {
  ...
  halo_list *last_hl = 0, *new_hl;
  halo *desc;
  memset(&halo_tree, 0, sizeof(halo_tree));
  ...
  qsort(last_hl->halos, last_hl->num_halos, sizeof(halo), sort_by_location);
  ...
  for (i=1; i<halo_tree.num_lists; i++) {
    ...
    qsort(new_hl->halos, new_hl->num_halos, sizeof(halo), sort_by_desc);
...
void read_tree(const char *filename) {
  ...
  while (fgets(buffer, 1024, input)) {
    ...
    h.desc = (halo *)(int64_t)descid;
    h.parent = (halo *)(int64_t)h.pid;
    h.uparent = (halo *)(int64_t)h.upid;
    if (!(all_halos.num_halos%3000))
      all_halos.halos = check_realloc(all_halos.halos, sizeof(halo)*(all_halos.num_halos+3000), "Allocating Halos.");
  ...
  all_halos.halos = check_realloc(all_halos.halos, sizeof(halo)*all_halos.num_halos, "Allocating Halos.");
  build_tree();
}
...
void delete_tree(void) {
  ...
  memset(&all_halos, 0, sizeof(halo_list));
}
